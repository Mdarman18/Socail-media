...
import { addPost as addPostApi } from "../../api/post.api";
...
export default function CreatePostModal() {
  ...
  const setResourceFromFile = (file) => {
    ...
    if (isImage) setRawImage(file);
    else setRawImage(null);
    const reader = new FileReader();
    reader.onload = () => {
      setResourceFile({
        file,
  ...
  const handlePreparePost = async () => {
    ...
    const finalPostData = {
      ...
      codeDetails:
        showCodeSnippet && codeDetails.trim() ? codeDetails : undefined,
      codeType: showCodeSnippet && codeDetails.trim() ? codeType : undefined,
    ...
    try {
      ...
      dispatch(addPost(newPost.post || newPost));
      dispatch(setIsCreateModalOpen(false));
    } catch (error) {
      setErrorMessage(
